...
#define ASSERTION_FAILED(msg) Serial.println(F(msg))

#endif

int combine_size_data = (6+3+3+4+16);
...
uint8_t* combinedArray = (uint8_t*)calloc(combine_size_data*4, sizeof(uint8_t));
// uint8_t byte_array



// float combinedArray[combine_size_data];
...
void sendTo(uint8_t* dataBuffer, size_t dataSize, IPAddress ip, uint16_t port) {
    udp.beginPacket(ip, port); // Start the UDP packet
    udp.write(dataBuffer, dataSize); // Send the raw byte array
    udp.endPacket(); // Complete the UDP packet
}
...
void convert_Measurment_to_byte(){
    ...
    save_ch();
    appendFloatsToBytes(combinedArray, imu_data, 6, currentIndex);
    appendFloatsToBytes(combinedArray, mag_data, 3, currentIndex);
    appendFloatsToBytes(combinedArray, quaternion_data, 4, currentIndex);
    appendFloatsToBytes(combinedArray, euler_data, 3, currentIndex);
    appendIntsToBytes(combinedArray, rc_ch_data, 16, currentIndex);
    printCombined();
...
void appendFloatsToBytes(uint8_t* dest, float* src, int count, int& offset) {
    for (int i = 0; i < count; i++) {
        memcpy(dest + offset, &src[i], sizeof(float)); // Copy float bytes
        offset += sizeof(float);
    }
}

// Helper function to append ints to the byte array
void appendIntsToBytes(uint8_t* dest, int* src, int count, int& offset) {
    for (int i = 0; i < count; i++) {
        memcpy(dest + offset, &src[i], sizeof(int)); // Copy int bytes
        offset += sizeof(int);
    }
}
...
void printCombined() {
    ...
    for (size_t i = 0; i < (combine_size_data * 4); i++) {
    ...
    Serial.println(); // Add a newline after all bytes are printed
}
